...
2. **协议匹配**：请求路径决定转发目标，`/v1/chat/completions` → OpenAI，`/v1/messages` → Anthropic
...
| api_key | TEXT | NOT NULL | 后端 API Key（加密存储） |
...
|------|------|------|------|
| id | TEXT | PK | UUID |
| client_model | TEXT | NOT NULL | 客户端模型名 |
...
路由器的全局 API Key 通过环境变量 `ROUTER_API_KEY` 配置，不存储在数据库中。
...
Client 请求
  ...
  → [3] 查找后端服务：匹配 api_type 且 is_active=true
...
- 项目脚手架搭建（TypeScript + Fastify）
- SQLite 数据库初始化与迁移
- API Key 认证中间件
- OpenAI 格式代理（含 SSE 流式）
- Anthropic 格式代理（含 SSE 流式）
- 模型映射功能
- 请求日志记录
- Docker 构建配置
